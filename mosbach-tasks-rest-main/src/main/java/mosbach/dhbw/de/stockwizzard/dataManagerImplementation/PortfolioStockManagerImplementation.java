...
public class PortfolioStockManagerImplementation implements IPortfolioStockManager{
    ...
    public void decreasePortfolioStock(Double newCurrentValue, Double newBoughtValue, Double stockAmount, Integer portfolioId, String symbol){
        ...
        try {
            ...
            Logger.getLogger("DecreasePortfolioStockLogger").log(Level.SEVERE, "Error when decreasing a portfolio stock.", e);
            e.printStackTrace();
        } finally {
            try {
                if (stmt != null) stmt.close();
                ...
                Logger.getLogger("DecreasePortfolioStockLogger").log(Level.SEVERE, "Error when closing the resource. Error: {0}", e);
                e.printStackTrace();
            }
        }
    }
    ...
    public void deletePortfolioStock(String symbol, Integer portfolioId) {
        ...
        try {
            ...
            Logger.getLogger("DeletePortfolioStockLogger").log(Level.SEVERE, "Error when deleting a portfolio stocks.", e);
            e.printStackTrace();
        } finally {
            try {
                if (stmt != null) stmt.close();
